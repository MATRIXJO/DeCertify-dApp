...
// Proxy download endpoint for IPFS
const proxyDownloadFromIPFS = async (req, res) => {
  const { ipfsHash } = req.params;
  if (!ipfsHash) {
    return res.status(400).json({ message: 'Missing IPFS hash.' });
  }
  try {
    const url = `https://gateway.pinata.cloud/ipfs/${ipfsHash}`;
    const response = await axios.get(url, { responseType: 'stream' });
    res.setHeader('Content-Disposition', `attachment; filename="${ipfsHash}.pdf"`);
    res.setHeader('Content-Type', response.headers['content-type'] || 'application/pdf');
    response.data.pipe(res);
  } catch (err) {
    console.error('Error proxying IPFS download:', err.message);
