...
const requestCertificate = async (req, res) => {
  const { organizationId, issuanceAmount, usn, yearOfGraduation, certificateType } = req.body;
...
// @desc    Publicly verify a certificate by IPFS hash
// @route   GET /api/verify-certificate/:ipfsHash
// @access  Public
const publicVerifyCertificate = async (req, res) => {
  const { ipfsHash } = req.params;
  if (!ipfsHash || typeof ipfsHash !== 'string') {
    return res.status(400).json({ valid: false, message: 'Invalid IPFS hash.' });
  }
  try {
    // Find the certificate request with this IPFS hash and status 'issued'
    const cert = await CertificateRequest.findOne({ ipfsHash, status: 'issued' })
      .populate('student', 'name email')
      .populate('organization', 'name email');
    if (!cert) {
      return res.status(404).json({ valid: false, message: 'Certificate not found or not issued.' });
    }
    // Optionally, check for revocation if you add that feature in the future
    res.json({
      valid: true,
      message: 'Certificate is authentic.',
      certificate: {
        student: cert.student?.name || null,
        studentEmail: cert.student?.email || null,
        organization: cert.organization?.name || null,
        organizationEmail: cert.organization?.email || null,
        usn: cert.usn,
        yearOfGraduation: cert.yearOfGraduation,
    ...
    console.error('Error verifying certificate by IPFS hash:', error);
