  ...
  const handleSubmit = async (e) => {
    ...
    if (isRegisterPage) {
      ...
      if (userType === 'organization') {
        if (issuanceFeeInput && (isNaN(issuanceFeeInput) || Number(issuanceFeeInput) < 0)) {
          toast.error('Issuance fee must be a positive number.');
          return;
        }
        if (verificationChargeInput && (isNaN(verificationChargeInput) || Number(verificationChargeInput) < 0)) {
          toast.error('Verification charge must be a positive number.');
          return;
        }
      }
      setBlockchainRegistrationLoading(true);
      try {
